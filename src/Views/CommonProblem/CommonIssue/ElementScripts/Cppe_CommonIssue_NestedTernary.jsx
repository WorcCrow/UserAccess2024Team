...
import useModalState from "./Cppe_CommonIssue_UseModalState";
//2 Issue
...
const NestedTernary = () => {
  ...
  const [issue1Visible, showIssue1, hideIssue1] = useModalState();
  const [issue2Visible, showIssue2, hideIssue2] = useModalState();
  const [issue3Visible, showIssue3, hideIssue3] = useModalState();
  const [issue4Visible, showIssue4, hideIssue4] = useModalState();
  const [issue5Visible, showIssue5, hideIssue5] = useModalState();
  const [issue6Visible, showIssue6, hideIssue6] = useModalState();
  ...
  return (
    <div className="d-flex justify-content-center">
      ...
      {issue1Visible && <DirectlyModifyingStateModal HideModal={hideIssue1} />}
      {issue2Visible && (
      ...
      {issue4Visible && <IncorrectlyUpdateState HideModal={hideIssue4} />}
      {issue5Visible && <ExpensiveRender HideModal={hideIssue5} />}
      {issue6Visible && <NotCleaningSideEffect HideModal={hideIssue6} />}
    </div>
  );
};
